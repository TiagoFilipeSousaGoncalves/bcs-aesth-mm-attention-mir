...
def create_dataframe(qns_list):
    ...
    for qns in qns_list:
        data['PID'].append(qns.query_vector_id)
        data['image PID'].append(qns.query_vector)  # Assuming query_vector stores the image path or ID
        data['neighbors'].append(qns.neighbor_vectors_id)

    df = pd.DataFrame(data)
    return df


def sample_manager(samples_path, pickle_path, catalogue_info, catalogue_user_info, patient_info, favorite_image_info, patient_images_info, catalogue_type='E', doctor_code=-1, split_ratio=0.8, default=False):

    if default:
        print('Reading Samples...')
        QNS_image_list, QNS_image_count = get_query_neighbor_elements_path(catalogue_info, catalogue_user_info, patient_info, favorite_image_info, patient_images_info, catalogue_type=catalogue_type, doctor_code=doctor_code)
        ...
        print('Loading QNS...')
        with open(f'{pickle_path}qns_list_train_F.pkl', 'rb') as file:
            QNS_image_list_train = pickle.load(file)
        with open(f'{pickle_path}qns_list_test_F.pkl', 'rb') as file:
            QNS_image_list_test = pickle.load(file)

        with open(f'{pickle_path}qns_list_train_F_tab.pkl', 'rb') as file:
            QNS_tabular_list_train = pickle.load(file)
        with open(f'{pickle_path}qns_list_test_F_tab.pkl', 'rb') as file:
            QNS_tabular_list_test = pickle.load(file)

    # Create DataFrame
    df_train = create_dataframe(QNS_image_list_train)
    df_test = create_dataframe(QNS_image_list_test)
