...
test_dataset = SegmentationDataset(
    image_dir=image_dir, 
    mask_dir=mask_dir, 
    image_transforms=val_image_transform,
    mask_transforms=val_mask_transform
)
...
def validate_model(model, dataloader, mse_loss):
    ...
    with torch.no_grad():
        for data, target, img_filenames, mask_filenames in dataloader: #img_filename and mask need to be here to unpack
            data = data.to(device)
...
def train_model(model_name, model, train_loader, val_loader, train_loss, optimizer, lr_scheduler, num_epochs, patience=5):
    ...
    for epoch in range(num_epochs):
        ...
        for i_step, (data, target, img_filenames, mask_filenames) in enumerate(train_loader):
            ...
            outputs = torch.sigmoid(outputs)  # Assumes the output of the model are logits
...
def test_model(model, dataloader, mse_loss, device, save_dir='/nas-ctm01/datasets/private/CINDERELLA/Pred_masks/', save_every_n_batches=1):
        ...
        for i, (data, target, img_filenames, mask_filenames) in enumerate(dataloader):
            ...
            if (i + 1) % save_every_n_batches == 0:
                for j in range(data.size(0)):
                    ...
                    original_image = (original_image * 255).astype(np.uint8)  # Assuming the image is normalized
                    ...
                    if contours:
                        ...
                        cropped_img_save_path = os.path.join(save_dir, f'cropped_{img_filenames[j]}')
