...
def train_triplets(model, train_loader, test_loader, QNS_list_train, QNS_list_test, optimizer, criterion, num_epochs, model_name, device='cpu', path_save='../bin/'):
    ...
    with open(f'{path_save}{model_name}/Train_info.log', 'a') as f:
        ...
        end_time = time.time()  # End time of the training process
        total_duration = end_time - start_time  # Total duration of the training process
        print(f'Total Training Duration: {total_duration:.2f} seconds')  # Print total duration
        print('Finished Training')
